	...
	$.ajax({
	    ...
	    success: function(postList) {
	    	...
	    	for (var i = 0; i < postList.length; i++) {
	    		...
	    		str += "<td><a href='/board/read?boardType=" + boardType + 
	    				"&boardSubType=" + boardSubType +
	    				"&currentPageNum=" + currentPageNum +
	    				"&amountData=" + amountData +
	    				"&postId=" + postList[i].postId + "'>" + postList[i].title + "</a></td>";
...
function createBoardPagination(amountPost) {
	...
	if(startPage != 1) {
		str += '<li class="paginateLi"><a href="' + (startPage - 1) + '" aria-label="Previous">';
	...
	if(endPage != realEndPage) {
		str += '<li class="paginateLi"><a href="' + (endPage + 1) + '" aria-label="Next">';
	...
	$('.paginateLi a').on("click", function(event) {
		...
		boardCriteriaFormGoList.find("input[name='currentPageNum']").val($(this).attr("href"));
