	...
	$.ajax({
	    ...
	    success: function(commentList) {
	    	...
	    	for (var i = 0; i < commentList.length; i++) {
	    		str += "<div>";
	    		str += "<h5 style='display: inline; padding-left: 10px'>" + commentList[i].writerId + "</h5>";
	    		
	    		if(readerId == commentList[i].writerId) { // 댓글 작성자가 읽고 있다면
	    			str += "<div class='btn-group pull-right'>";
	    			str += "<button type='button' class='btn btn-default dropdown-toggle' data-toggle='dropdown' style='border: 0; outline: 0; padding: 0px' aria-expanded='false'>";
	    			str += "<span class='glyphicon glyphicon-option-vertical' aria-hidden='true'></span>";
	    			str += "</button>";
	    			str += "<input type='hidden' value='" + commentList[i].commentId + "' />";
	    		...
	    		str += "<br>";
	    		str += "<h5 style='color: DarkGray; display: inline-block'>" + commentList[i].regdate + "</ht>";
	    		str += "<h5 class='pull-right' style='margin-right: 5px; color: DarkGray'>" + commentList[i].heartCount + "</h5>";
	    		str += "<p style='background-color: aliceblue; padding: 10px; margin-bottom: 10px'>" + commentList[i].content + "</p>";
	    		str += "</div>";
	    	}
    	    ...
    	    $(".updateComment").on('click', function (event) {
    	    	...
    	    	str += '<div>';
    	    	str += '<textarea id="modifyCommentContent" class="form-control" rows="5"></textarea>';
    	    	str += '<div style="width: 100%; text-align: right; margin-bottom: 10px">';
    	    	str += '<button type="button" id="modifyCommentRegister" class="btn btn-primary">수정</button>';
    	    	str += '</div></div>';
